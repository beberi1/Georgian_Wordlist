...
# არ არის დასრულებული არ არის სრული ფუნქციონალი
# არ ვარგა მოკლედ ჯერ
...
# შეკითხვების ნაწილი
if input("გინდათ ყველაფერი გაუშვათ? *კომპიუტერს გაუჭირდება* (ki/ara): ").strip().lower() == 'ki':
    print("თქვენი პასუხი იყო კი")
    ...
    print("თქვენი პასუხი იყო არა")

print("ახლა გადავიდეთ შეკითხვებზე თუ როგორ გინდათ რომ დაამუშავოთ შეყვანილი სიტყვა")


if input("გინდათ დიდი და პატარა ასოების კომბინაციების გაკეთება? (ki/ara): ").strip().lower() == 'ki':
    print("თქვენი პასუხი იყო კი")
    upper_and_lowercase(saxeli, output_file)
else:
    print("თქვენი პასუხი იყო არა")
...
if input("გინდათ კომბინაციები სადაც ზოგი ასო სიმბოლოებით იცვლება? (ki/ara): ").strip().lower() == 'ki':
    print("თქვენი პასუხი იყო კი")
    process_words(input_file, output_file, 'i', '!')
    process_words(input_file, output_file, 'e', '3')
    process_words(input_file, output_file, 'o', '0')
...
if input("გინდათ კომბინაციები სადაც რიცხვები ემატება შეყვანილ სიტყვას თავში და ბოლოში? (ki/ara): ").strip().lower() == 'ki':
    print("თქვენი პასუხი იყო კი")
    output_file = r'pass1.txt'  
    fill_with_numbers(input_file, output_file)
else:
...
if input("გინდათ კომბინაციები სადაც წლები ემატება შეყვანილ სიტყვას თავში და ბოლოში? (ki/ara): ").strip().lower() == 'ki':
    print("თქვენი პასუხი იყო კი")
    output_file = r'pass2.txt'  
    append_years_to_words(input_file, output_file)
else:
...
print("ამ ფუნქციის სამუშაოდ საჭიროა ჯერ გაუშვათ All_Geo_numbers.py ფაილი | გაფრთხილება: კომპიუტერს გაუჭირდება ამ კოდის გაშვება")
if input("გინდათ ყველა (2024) წლისთვის არსებული ნომერი დავამატოთ უკნიდან შეყვანილ სიტყვას? (ki/ara): ").strip().lower() == 'ki':
    print("თქვენი პასუხი იყო კი")

else:
    print("თქვენი პასუხი იყო არა")

print("შენიშვნა: ეს არ მუშაობს ნომრებიან ფაილზე")
if input("გინდათ საბოლოოდ მიიღოთ ერთი ფაილი? (ki/ara): ").strip().lower() == 'ki':
    print("თქვენი პასუხი იყო კი")
    file1="pass.txt"
    file2="pass1.txt"
    file3="pass2.txt"
